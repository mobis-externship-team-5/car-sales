...
UINT getHashValue(const int key)
{	
	int hashValue = key;
	
	hashValue = ((hashValue >> 16) ^ hashValue) * HASH_MAGIC_CODE;
    hashValue = ((hashValue >> 16) ^ hashValue) * HASH_MAGIC_CODE;
...
LPNODE nodeCreate(const int key, LPDATA value)
{
	LPNODE lpNode;
...
int hashRemoveKey(LPHASH lpHash, const int key)
{
	...
	UINT hashValue = getHashValue(key); //key에 대한 hash value값을 생성한다.
	int nHashBucket;
